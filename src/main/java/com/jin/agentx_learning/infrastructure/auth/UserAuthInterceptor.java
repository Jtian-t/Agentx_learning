package com.jin.agentx_learning.infrastructure.auth;
...
//    当 preHandle 判定一个请求非法（没带 Token 或 Token 错误）并返回 false 时，
//    Spring MVC 的流程就中断了，请求根本进不去你的 Controller。这时候，为了让前端知道发生了什么，
//    你需要手动通过 HttpServletResponse 把错误信息“塞”给用户。
    /** 写入错误响应
    ...
    private void writeErrorResponse(HttpServletResponse response, String message) {
        try {
            ...
            //TODO 虽然 String.format 这种手动拼字符串的方法能跑通，但在大型项目里，开发者通常会更倾向于使用 Jackson 或 Gson 库：
            //手动拼字符串最怕的就是 message 里带有双引号，会导致生成的 JSON 格式直接报废。
            //// 更专业的写法（防止手动拼 JSON 字符串时引号、逗号出错）
            //String jsonResponse = new ObjectMapper().writeValueAsString(resultObject);
